...
export const AuthProvider = ({ children }) => {
    ...
    useEffect(() => {
        const userToken = localStorage.getItem("user_token"); // usar localStorage do navegador para salvar dados da sessão do usuário
        const usersStorage = localStorage.getItem("user_db");

        if (userToken && usersStorage) {
            const hasUser = JSON.parse(usersStorage)?.filter(
                (user) => user.email === JSON.parse(userToken).email // verificar se o usuário tem o mesmo e-mail que o token
            );
            if (hasUser) {
                setUser(hasUser[0]); // salvar no state o usuário encontrado
            }
        }
    }, []);
